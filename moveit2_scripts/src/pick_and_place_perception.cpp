...
int main(int argc, char **argv) {
  ...
  auto find_objects_client = std::make_shared<FindObjectsClient>();
  find_objects_client->sendGoal();
  while (!find_objects_client->getResult()) {
    rclcpp::spin_some(find_objects_client);
  }

  geometry_msgs::msg::Pose pose_from_perception{
      *(find_objects_client->getResult())},
      target_pose1;
  target_pose1.position.x = pose_from_perception.position.x;
  target_pose1.position.y = pose_from_perception.position.y;
  target_pose1.position.z = 0.24;
  // rotate pose by 180 degrees around the x axis of the inertial frame
  target_pose1.orientation.x = pose_from_perception.orientation.w;
  target_pose1.orientation.y = -pose_from_perception.orientation.z;
  target_pose1.orientation.z = pose_from_perception.orientation.y;
  target_pose1.orientation.w = -pose_from_perception.orientation.x;
